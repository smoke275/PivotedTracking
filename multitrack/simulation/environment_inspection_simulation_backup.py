...
def calculate_dynamic_visibility_range(agent1_pos, agent2_pos, time_horizon, leader_speed):
    """
    Consolidated dynamic range calculation logic.
    Eliminates duplicate dynamic range calculations.
    
    Returns:
        float: calculated visibility range
    """
    # DYNAMIC RANGE CALCULATION: Agent 1 reachability + inter-agent distance (max 800px)
    max_reachable_distance = time_horizon * leader_speed
    inter_agent_distance = math.sqrt((agent1_pos[0] - agent2_pos[0])**2 + (agent1_pos[1] - agent2_pos[1])**2)
    dynamic_range = min(800, max_reachable_distance + inter_agent_distance)
    return dynamic_range

def get_rod_colors():
    """
    Consolidated rod color definitions.
    Eliminates duplicate color array definitions.
    
    Returns:
        list: standardized rod colors
    """
    return [
        (255, 0, 0),    # Red
        (0, 255, 0),    # Green  
