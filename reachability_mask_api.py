...
class ReachabilityMaskAPI:
    ...
    def __init__(self, filename_base: str = "unicycle_grid"):
        """Initialize API. Args: filename_base: Base name of pickle file (without .pkl)"""
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            if self._mask_data is None:
                print(f"Warning: Loaded mask data is None from {self.filename_base}.pkl")
                return False
            
            self._grid = self._mask_data.get('grid')
            if self._grid is None:
                print(f"Warning: No grid data found in {self.filename_base}.pkl")
                return False
            ...
            print(f"✅ Reachability mask loaded: {self._grid_size}×{self._grid_size} grid")
            return True
            
        except FileNotFoundError:
            print(f"❌ File not found: {self.filename_base}.pkl")
            print("Run heatmap.py first to generate the reachability mask.")
            return False
    ...
    def configure_processed_overlay(self, clip_pixels: float = 32.0, 
        ...
        if not self.is_loaded():
            print("⚠️  Cannot configure overlay: no reachability mask loaded")
            return False
        ...
        if (self._processed_overlay is not None and 
            ...
            print(f"✅ Processed overlay already configured: {self._processed_overlay['processing_chain']}")
            return True
        
        # Compute the processed overlay
        print(f"🔄 Computing processed overlay: {self._grid_size}×{self._grid_size} → clip({clip_pixels}px) → {target_size[0]}×{target_size[1]} → {upscale_target[0]}×{upscale_target[1]}")
        if probability_weighting is not None:
            print(f"   🎲 Probability weighting: Prelect(α={probability_weighting[0]:.2f}, β={probability_weighting[1]:.2f})")
        ...
        if 'error' in processed_result:
            print(f"❌ Failed to configure processed overlay: {processed_result['error']}")
            self._processed_overlay = None
        ...
        print(f"✅ Processed overlay configured: {pipeline_info}")
        print(f"   📐 Scale factor: {scale_factor:.2f}× ({quality_info['interpolation_method']})")
        print(f"   🎯 Quality: {quality_info['pixel_to_grid_ratio']} {quality_info['coordinate_mapping']}")
    ...
    def get_cached_overlay(self) -> Optional[Dict[str, Any]]:
        ...
        if self._processed_overlay is None:
            print("⚠️  No processed overlay configured. Call configure_processed_overlay() first.")
            return None
    ...
    def _calculate_grid_statistics(self, grid: np.ndarray, grid_size: int) -> Dict[str, Any]:
        ...
        # Statistics for reachable cells only
        if reachable_cells > 0:
            reachable_values = grid[non_zero_mask]
            mean_reachable = float(np.mean(reachable_values))
            std_reachable = float(np.std(reachable_values))
            min_reachable = float(np.min(reachable_values))
            max_reachable = float(np.max(reachable_values))
        else:
            mean_reachable = 0.0
            std_reachable = 0.0
            min_reachable = 0.0
    ...
    def get_summary_info(self) -> str:
        ...
        summary = f"🎯 Reachability Mask Summary: {self.filename_base}.pkl\n"
        summary += f"{'=' * 50}\n"
        summary += f"📊 Grid: {stats['grid_size']}×{stats['grid_size']} cells\n"
        summary += f"🌍 World extent: ±{stats['world_extent_px']/2:.1f} pixels\n"
        summary += f"📏 Cell size: {stats['cell_size_px']:.3f} px/cell\n"
        summary += f"🎯 Center: grid[{stats['center_idx']}, {stats['center_idx']}] = world(0, 0)\n"
        summary += f"\n📈 Coverage:\n"
        summary += f"  ✅ Reachable: {stats['reachable_cells']:,} cells ({stats['reachable_percentage']:.1f}%)\n"
        summary += f"  ❌ Unreachable: {stats['unreachable_cells']:,} cells\n"
        summary += f"\n🔢 Values:\n"
        summary += f"  Range: {stats['min_value']:.6f} to {stats['max_value']:.6f}\n"
        summary += f"  Mean (reachable): {stats['mean_reachable']:.6f}\n"
        summary += f"  Std (reachable): {stats['std_reachable']:.6f}\n"
        summary += f"\n🏆 Maximum: {stats['max_value']:.6f}\n"
        summary += f"  Location: grid{stats['max_location_grid']} = world({stats['max_location_world'][0]:.1f}, {stats['max_location_world'][1]:.1f})\n"
    ...
    def _apply_prelect_probability_weighting(self, grid: np.ndarray, alpha: float, beta: float) -> np.ndarray:
        ...
        grid_clamped = np.clip(grid, 1e-10, 1.0 - 1e-10)  # Avoid exact 0 and 1 for numerical stability
        
        # Apply Prelect transformation: p^α / (p^α + (1-p)^β)
        p_alpha = np.power(grid_clamped, alpha)
        one_minus_p_beta = np.power(1.0 - grid_clamped, beta)
        
        # Compute Prelect-weighted probabilities
        weighted_grid = p_alpha / (p_alpha + one_minus_p_beta)
        
        return weighted_grid.astype(np.float32)
    ...
    def get_processed_overlay(self, clip_pixels: float = 32.0, 
        ...
        original_final_grid = final_grid.copy()  # Keep a copy for debugging
        ...
        if probability_weighting is not None:
            ...
            print(f"🎲 Applying Prelect probability weighting: α={alpha:.2f}, β={beta:.2f}")
            
            # Apply Prelect weighting to the final grid
            weighted_grid = self._apply_prelect_probability_weighting(final_grid, alpha, beta)
            
            # Calculate weighting statistics
            original_mean = np.mean(final_grid)
            weighted_mean = np.mean(weighted_grid)
            original_std = np.std(final_grid)
            weighted_std = np.std(weighted_grid)
            
            print(f"   📊 Original stats: mean={original_mean:.6f}, std={original_std:.6f}")
            print(f"   📊 Weighted stats: mean={weighted_mean:.6f}, std={weighted_std:.6f}")
            print(f"   📈 Mean change: {((weighted_mean - original_mean) / original_mean * 100):.2f}%")
            
            final_grid = weighted_grid
    ...
    def create_test_mask(self, shape: Tuple[int, int] = (400, 400), 
        ...
        # Create a test pattern with multiple features for visual distinction
        test_mask = np.zeros(shape, dtype=np.float32)
        
        # Create concentric circles pattern with varying intensities
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                ...
                # Create multiple zones with different values
                if dist < 50:
                    test_mask[i, j] = 1.0  # Center bright zone
                elif dist < 100:
                    test_mask[i, j] = 0.8  # Inner ring
                elif dist < 150:
                    # Checkerboard pattern in middle ring
                    if (i // 20 + j // 20) % 2 == 0:
                        test_mask[i, j] = 0.6
                    else:
                        test_mask[i, j] = 0.3
                else:
                    # Diagonal stripes in outer zone
                    if (i + j) % 30 < 15:
                        test_mask[i, j] = 0.4
                    else:
                        test_mask[i, j] = 0.1
        
        # Add some random high-value spots for additional visual interest
            ...
            for dx in range(-5, 6):
                for dy in range(-5, 6):
                    if 0 <= x+dx < shape[0] and 0 <= y+dy < shape[1]:
                        if dx*dx + dy*dy <= 25:  # Circle of radius 5
                            test_mask[x+dx, y+dy] = min(1.0, test_mask[x+dx, y+dy] + 0.5)
        ...
        print(f"🧪 Created test mask: {test_mask.shape[0]}×{test_mask.shape[1]} with complex pattern")
        print(f"   📊 Test mask range: [{test_mask.min():.3f}, {test_mask.max():.3f}]")
        print(f"   🎨 Pattern: concentric circles + checkerboard + stripes + bright spots")
        print(f"   ➡️  Arrow: LARGE prominent arrow drawn LAST (on top) pointing towards positive x-axis")
    ...
    def transform_mask(self, mask_data: Tuple[np.ndarray, Dict[str, float]], 
        ...
        if abs(relative_offset_x) > 1e-6 or abs(relative_offset_y) > 1e-6:
            print(f"📍 Applying translation: agent({agent_x:.1f}, {agent_y:.1f}) → target({target_x:.1f}, {target_y:.1f})")
            print(f"   📏 Relative offset: ({relative_offset_x:.2f}, {relative_offset_y:.2f}) = ({offset_x_pixels:.1f}, {offset_y_pixels:.1f}) pixels")
            ...
            # No relative offset - mask stays centered on agent
            translated_mask = rotated_mask.copy()
            if translated_mask.dtype != np.float32:
                translated_mask = translated_mask.astype(np.float32)
            print(f"📍 No translation needed: target location matches agent location ({target_x:.1f}, {target_y:.1f})")
        
        # Step 4: Apply visibility polygon clipping if provided
        if visibility_polygon is not None and len(visibility_polygon) > 2:
            print(f"🔍 Applying visibility polygon clipping with {len(visibility_polygon)} vertices")
            print(f"   📐 World bounds: x=[{world_bounds['x_min']:.1f}, {world_bounds['x_max']:.1f}], y=[{world_bounds['y_min']:.1f}, {world_bounds['y_max']:.1f}]")
            print(f"   📍 Target location: ({target_location[0]:.1f}, {target_location[1]:.1f})")
            print(f"   👁️  Visibility polygon bounds: x=[{min(p[0] for p in visibility_polygon):.1f}, {max(p[0] for p in visibility_polygon):.1f}], y=[{min(p[1] for p in visibility_polygon):.1f}, {max(p[1] for p in visibility_polygon):.1f}]")
            ...
            print(f"   👁️  Efficient polygon rasterization: {inside_pixels}/{total_pixels} pixels ({visibility_percentage:.1f}% inside polygon)")
            print(f"   📊 Final clipped mask range: [{final_mask.min():.3f}, {final_mask.max():.3f}]")
            
            # If no pixels are inside polygon, create a small test pattern around the agent
            if inside_pixels == 0:
                print("   ⚠️  No pixels inside polygon - creating test pattern around agent")
                ...
                print(f"   🎯 Created test circle at grid ({center_j}, {center_i}) with radius {radius}")
                inside_pixels = np.sum(final_mask != 0.0)
                print(f"   ✅ Test pattern created: {inside_pixels} pixels")
            
            translated_mask = final_mask
        else:
            print(f"   ⚠️  No visibility polygon provided - using full transformed mask")
        ...
        print(f"🔄 Transformed mask:")
        print(f"   📐 Rotated by {rotation_degrees:.1f}° ({orientation:.3f} rad) using {rotation_method}")
        print(f"   📍 Translated to ({target_x:.1f}, {target_y:.1f})")
        print(f"   🔄 Y-axis flipped for matplotlib compatibility")
        print(f"   📊 Final mask range: [{translated_mask.min():.3f}, {translated_mask.max():.3f}]")
    ...
    def create_canvas_overlay(self, reachability_data: Optional[Tuple[np.ndarray, Dict[str, Any]]] = None) -> Optional[Tuple[np.ndarray, Dict[str, Any]]]:
        ...
        print(f"🎨 Created canvas overlay: {canvas_mask.shape[0]}×{canvas_mask.shape[1]} filled with 0s")
        print(f"   📐 Canvas dimensions match processed overlay")
    ...
    def apply_mask_to_canvas(self, canvas_data: Optional[Tuple[np.ndarray, Dict[str, Any]]], 
        ...
        if canvas_mask.shape != mask.shape:
            print(f"⚠️  Shape mismatch: canvas {canvas_mask.shape} vs mask {mask.shape}")
            return canvas_data  # Return canvas unchanged
        
        # Element-wise maximum: canvas = max(canvas, mask)
        final_canvas = np.maximum(canvas_mask, mask)
        
        print(f"🎯 Applied mask to canvas: {final_canvas.shape[0]}×{final_canvas.shape[1]}")
        print(f"   📊 Final canvas range: [{final_canvas.min():.3f}, {final_canvas.max():.3f}]")
    ...
    def _rasterize_polygon_scanline(self, vertices, height, width):
        ...
        if len(vertices) < 3:
            return np.zeros((height, width), dtype=np.uint8)  # Use uint8 for C++ compatibility
        
        # Convert to numpy arrays for efficient memory layout (C++ friendly)
        vertices_array = np.array(vertices, dtype=np.float32)
        
        # Pre-allocate output mask with C++ compatible type
        mask = np.zeros((height, width), dtype=np.uint8)
        
        # Convert vertices to integer coordinates using vectorized operations
        int_vertices = np.round(vertices_array).astype(np.int32)
        
        # Ensure polygon is closed
        if not np.array_equal(int_vertices[0], int_vertices[-1]):
            int_vertices = np.vstack([int_vertices, int_vertices[0:1]])
        
        # Build edge table using structured arrays (C++ struct-like)
        edge_dtype = np.dtype([
            ('y_min', np.int32),
            ('y_max', np.int32), 
            ('x_start', np.int32),
        ...
        max_edges = len(int_vertices) - 1
        edges = np.zeros(max_edges, dtype=edge_dtype)
        edge_count = 0
        
        # Build edges using direct array operations
        for i in range(len(int_vertices) - 1):
            x1, y1 = int_vertices[i]
            x2, y2 = int_vertices[i + 1]
            ...
            dy = y2 - y1
            if dy != 0:
                dx_dy = float(x2 - x1) / float(dy)
                edges[edge_count] = (y1, y2, x1, dx_dy)
                edge_count += 1
        
        # Trim edges array to actual size
        ...
        # Get scanline bounds
        y_min = int(np.min(edges['y_min']))
        y_max = int(np.max(edges['y_max']))
        y_min = max(0, y_min)
        y_max = min(height, y_max + 1)
        
        # Pre-allocate intersection buffer (reuse for efficiency)
        max_intersections = edge_count
        intersections = np.zeros(max_intersections, dtype=np.float32)
        
        # Process each scanline with optimized inner loop
        for y in range(y_min, y_max):
            intersection_count = 0
            
            # Find intersections using vectorized operations where possible
            for edge_idx in range(edge_count):
                edge = edges[edge_idx]
                if edge['y_min'] <= y < edge['y_max']:
                    x_intersect = edge['x_start'] + (y - edge['y_min']) * edge['dx_dy']
            ...
            # Sort intersections (in-place for efficiency)
            intersections_slice = intersections[:intersection_count]
            intersections_slice.sort()
            
            # Fill between pairs of intersections
            for i in range(0, intersection_count, 2):
                if i + 1 < intersection_count:
                    x_start = max(0, int(np.round(intersections_slice[i])))
                    x_end = min(width, int(np.round(intersections_slice[i + 1])) + 1)
                    
                    if x_start < x_end:
                        mask[y, x_start:x_end] = 1  # Use 1 instead of True for C++ compatibility
        
        return mask.astype(np.bool_)  # Convert back to boolean for Python compatibility
    ...
    def generate_reachability_path_data(self, 
        ...
        print("🔄 Setting up reachability overlay...")
        reachability_data = None
        sample_points_data = []
        
        try:
            if not self.is_loaded():
                print(f"⚠️ Failed to load reachability data from {self.filename_base}.pkl")
                return None, []
            
            if not self.is_overlay_configured():
                print(f"⚠️ Overlay not configured. Call setup_overlay_configuration() first.")
                return None, []
            ...
            if not original_reachability_data:
                print(f"⚠️ Failed to get original reachability data")
                return None, []
            ...
            if isinstance(original_reachability_data, tuple) and len(original_reachability_data) >= 2:
                ...
                print(f"⚠️ Unexpected original_reachability_data format")
                return None, []
            ...
            if isinstance(test_mask_data, tuple) and len(test_mask_data) >= 2:
                ...
                print(f"⚠️ Unexpected test_mask_data format")
                return None, []
            
            # Only proceed if we have valid data
            if original_mask is None or original_bounds is None or test_mask is None or test_world_bounds is None:
                print(f"⚠️ Missing required data components for reachability overlay")
                return None, []
            
            # Transform test data to agent location and orientation
            print(f"🔄 Calling transform_mask...")
            print(f"🔍 Debug: Using test_world_bounds instead of original_bounds")
            print(f"   Original bounds: {original_bounds}")
            print(f"   Test mask bounds: {test_world_bounds}")
            ...
            print(f"✅ Transform_mask completed successfully")
            print(f"🔍 Debug: transformed_test_data type: {type(transformed_test_data)}")
            if isinstance(transformed_test_data, tuple):
                print(f"🔍 Debug: transformed_test_data tuple length: {len(transformed_test_data)}")
            ...
            print(f"🔄 Creating masks for {len(path_analysis_data)} paths...")
            ...
            for path_idx, path_info in enumerate(path_analysis_data):
                ...
                if target_point is None or path_orientation is None or path_polygon is None:
                    print(f"  ⚠️ Path {path_idx+1}: Missing data (target_point={target_point is not None}, orientation={path_orientation is not None}, polygon={path_polygon is not None})")
                    continue
                ...
                if transformed_test_data is not None and path_info.get('first_edge'):
                    try:
                        ...
                        if isinstance(transformed_test_data, tuple) and len(transformed_test_data) >= 2:
                            ...
                            grid_x = int((sample_point[0] - grid_x_min) / (grid_x_max - grid_x_min) * grid_width)
                            grid_y = int((sample_point[1] - grid_y_min) / (grid_y_max - grid_y_min) * grid_height)
                            ...
                            print(f"  🔍 Value at sample point {sample_point}: {target_value:.6f} (grid coords: {grid_x}, {grid_y})")
                            print(f"      (moved {edge_length:.1f} + 15.0 units from edge start {edge_start} towards circle center)")
                            ...
                            print(f"  🔍 transformed_test_data format unexpected: {type(transformed_test_data)}")
                    except Exception as sample_error:
                        print(f"  ⚠️ Error sampling transformed_test_data along first edge: {sample_error}")
                
                print(f"  🎯 Path {path_idx+1}: Creating mask for target {target_point}, orientation {math.degrees(path_orientation):.1f}°")
                ...
                try:
                    ...
                    if target_value is not None and target_value != 0:
                        ...
                        print(f"  🔢 Multiplying original reachability data by boosted value: {target_value:.6f} → {boosted_value:.6f} (+30%, capped at 1.0)")
                        original_mask = original_reachability_data[0]
                        original_bounds = original_reachability_data[1]
                        ...
                        print(f"  ✅ Modified reachability data created (values scaled by {boosted_value:.6f})")
                    else:
                        print(f"  ⚠️ Skipping multiplication (target_value={target_value})")
                    ...
                    if path_transformed_data is not None:
                        ...
                        if temp_overlay:
                            ...
                            print(f"    ✅ Path {path_idx+1}: Mask applied successfully")
                        else:
                            print(f"    ⚠️ Path {path_idx+1}: Failed to apply mask to canvas")
                    else:
                        print(f"    ⚠️ Path {path_idx+1}: Transform_mask returned None")
                        
                except Exception as path_error:
                    print(f"    ❌ Path {path_idx+1}: Error during mask transformation: {path_error}")
                    continue
            
            # Update reachability_data with the final overlay
            if updated_overlay:
                reachability_data = updated_overlay
                print(f"✅ All path masks applied. Final overlay has {len(path_analysis_data)} path contributions.")
            
            # Apply transformed test data to canvas (original behavior)
            if transformed_test_data is not None:
                print(f"🔄 Calling apply_mask_to_canvas...")
                final_overlay = self.apply_mask_to_canvas(updated_overlay, transformed_test_data)
                if final_overlay:
                    reachability_data = final_overlay
                    print(f"✅ Reachability overlay configured and applied with test data")
                    print(f"🔍 Debug: final reachability_data type: {type(reachability_data)}")
                    if isinstance(reachability_data, tuple):
                        print(f"🔍 Debug: final reachability_data tuple length: {len(reachability_data)}")
                else:
                    print(f"⚠️ Failed to apply transformed test data to canvas")
            else:
                print(f"⚠️ Transform_mask returned None for test data")
                
        except Exception as transform_error:
            print(f"⚠️ Error during mask transformation: {transform_error}")
            import traceback
            traceback.print_exc()
            reachability_data = None
    ...
    def process_paths_with_reachability(self,
        ...
        print("🔄 Processing path analysis and reachability overlay...")
        
        # Check if overlay is configured
        if not self.is_overlay_configured():
            print("⚠️ Overlay not configured. Call setup_overlay_configuration() first.")
            print("   Proceeding with path analysis only (no reachability overlay)...")
            
        # STEP 1: Compute path analysis data (moved from main function)
        print("🔄 Processing path first edges and polygons...")
        path_analysis_data = []
            ...
            if 'path_points' in path_data and len(path_data['path_points']) > 1:
                ...
                if path_segments and len(path_segments) > 0:
                    for seg_idx, segment in enumerate(path_segments):
                        ...
                        if segment['type'] == 'line':
                            ...
                            print(f"           Arc segment {seg_idx+1}: {segment_start} → {segment_end}")
                            print(f"             Center: {center}, Radius: {radius:.1f}")
                            print(f"             Start angle: {math.degrees(actual_start_angle):.1f}°, End angle: {math.degrees(actual_end_angle):.1f}°")
                            print(f"             Arc length: {arc_length:.3f} rad ({math.degrees(arc_length):.1f}°)")
                            print(f"             Linearized into {num_points} points")
                        ...
                        if current_position and seg_idx > 0:
                            ...
                            if distance_to_expected > 1e-6:  # Small tolerance for floating point errors
                                print(f"           ⚠️ Gap detected: previous end {expected_start} vs current start {segment_start} (distance: {distance_to_expected:.6f})")
                else:
                    # Fallback: use original points if no segments available
                    path_polygon = points.copy()
                    print(f"           Using original {len(points)} points (no segments available)")
                ...
                if first_edge:
                    ...
                    if first_edge['type'] == 'line':
                        print(f"  Path {i+1}: First edge is LINE from {first_edge['start']} to {first_edge['end']}")
                        print(f"           Target point (45° arrow start): {target_point}")
                        if orientation_deg is not None:
                            print(f"           Orientation: {orientation_deg:.1f}° (away from center)")
                        ...
                        print(f"  Path {i+1}: First edge is ARC from {first_edge['start']} to {first_edge['end']} "
                              f"(angles: {start_angle_deg:.1f}° to {end_angle_deg:.1f}°)")
                        print(f"           Target point (45° arrow start): {target_point}")
                        if orientation_deg is not None:
                            print(f"           Orientation: {orientation_deg:.1f}° (away from center)")
                else:
                    print(f"  Path {i+1}: No first edge found")
                ...
                if linearized_points > original_points:
                    ...
                    print(f"           Polygon linearized: {original_points} → {linearized_points} points")
                    print(f"           Path composition: {line_segments} line segments, {arc_segments} arc segments")
                    print(f"           Arc linearization added {linearized_points - original_points} intermediate points")
                else:
                    print(f"           Polygon points: {linearized_points} (no arcs to linearize)")
        
        print(f"✅ Processed {len(path_analysis_data)} paths with first edge analysis")
        ...
        if self.is_overlay_configured():
            ...
            print("⚠️ Skipping reachability overlay generation (overlay not configured)")
            reachability_data = None
            sample_points_data = []
...
if __name__ == "__main__":
    # Demo usage
    print("🎯 Reachability Mask API Demo")
    print("=" * 50)
    ...
    if api.is_loaded():
        # Print summary
        print(api.get_summary_info())
        
        # Demo some API calls
        print(f"\n🔍 Example API calls:")
        print(f"Value at center: {api.get_value_at_world(0, 0):.6f}")
        print(f"Value at (10, 10): {api.get_value_at_world(10, 10):.6f}")
        ...
        for analysis in distance_analysis[:3]:
            ...
            print(f"  {dist_range[0]:6.1f}-{dist_range[1]:6.1f}px: {coverage:5.1f}% coverage, avg: {avg_reach:.6f}")
    else:
        print("❌ Could not load reachability mask data")
